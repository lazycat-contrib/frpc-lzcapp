...
5. Adjust the initial `[[proxies]]` entry or create additional proxies dynamically from the UI.
...
`.github/workflows/lazycat.yml` publishes from `main` or `workflow_dispatch`. It uses mirror delivery with `require_digest_match: true`, keeps the Action image source on `ghcr.io/fatedier/frpc`, generates versioned release assets, and publishes only to the MiaoMiao private store.
